...
def _generation_already_processed(fs, path_to_generation):
    return fs.exists(path_to_generation + "/_SUCCESS")


def _get_sorted_dataset_generations(fs, base_path, app_name, app_version):
...
def _get_all_files_except_success_file(fs, path_to_generation):
...
def get_latest_dataset(fs, base_path, app_name, compatible_versions):
    for version in sorted([int(v) for v in compatible_versions.split(",")], reverse=True):
        ...
        for path_to_generation in fs.ls(path_to_version):
            if not _generation_already_processed(fs, path_to_generation):
                return _get_all_files_except_success_file(fs, path_to_generation)
