...
def test_generation_already_processed():
    assert r._generation_already_processed(test_fs, zipper_base_path + "/0001/0001")
    assert not r._generation_already_processed(test_fs, zipper_base_path + "/0002/0002")


def test_get_dataset_generations():
    assert r._get_sorted_dataset_generations(test_fs, base_path=fs_path, app_name="zipper", app_version="0001") == \
...
def test_get_all_dataset_files():
    assert r._get_all_files_except_success_file(test_fs, zipper_base_path + "/0001/0001") == \
           [zipper_base_path + "/0001/0001/dataset.csv"]
    assert r._get_all_files_except_success_file(test_fs, zipper_base_path + "/0001/0002") == []
